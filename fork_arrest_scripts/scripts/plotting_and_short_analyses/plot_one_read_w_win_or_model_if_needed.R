#!/usr/bin/env Rscript
# usage Rscript ./program_name.R plot_data plot.png plot_elements_this_is_optional plot_dims_res_etc_optional
...
# * option plot_dims_res_etc_optional must have the format width,height,base_size,dpi,colour_win,colour_model
#   e.g. 18,12,60,200,#000000,#FF0000. You can omit any value but if you omit one, you have to omit the next ones.
#   i.e. you cannot pass strings like 18,12,,#000000 but 18,12 is valid.
#   width and height are in inches, base_size is the font size, dpi is the resolution of the plot,
#   colour_win is the colour of the windowed detect data (irrespective of strand), and colour_model
#   is the colour of the model data. If you don't pass any value, the default values are
#   18,12,60,200,and colour blue or red for + or - orientation respectively, and black for the model data.
...
# set some default colours in the plot
colour_win_detect <- "#C61F16"
colour_plus <- "#1F68C4"
colour_plus_annotate <- "#1F68C4"
colour_minus <- "#C61F16"
colour_minus_annotate <- "#C61F16"
colour_model <- "#000000"
...
if (length(args) < 2) {
  stop("Usage: Rscript ./<script_name>.R plot_data plot.png plot_elements_this_is_optional plot_dims_res_etc_optional",
       call.= FALSE)
}

# process the annotation file if it exists
if (length(args) >= 3 && args[3] != "" && args[3] != "/dev/null" && file.size(args[3]) > 0) {
...
if (length(args) >= 4 && args[4] != "") {
  plot_dims_res_optional <- strsplit(args[4], ",")[[1]]

  if (length(plot_dims_res_optional) > 0) width <- as.numeric(plot_dims_res_optional[1])
  if (length(plot_dims_res_optional) > 1) height <- as.numeric(plot_dims_res_optional[2])
  if (length(plot_dims_res_optional) > 2) base_size <- as.numeric(plot_dims_res_optional[3])
  if (length(plot_dims_res_optional) > 3) dpi <- as.numeric(plot_dims_res_optional[4])
  if (length(plot_dims_res_optional) > 4) {
    colour_win_detect <- plot_dims_res_optional[5]
    colour_plus <- colour_win_detect
    colour_minus <- colour_win_detect
  }
  if (length(plot_dims_res_optional) > 5) colour_model <- plot_dims_res_optional[6]
}
...
# check if any entry in comments is "# do_not_plot_annotations"
# if it is, then set the flag is_override_plot_annotations to TRUE
is_override_plot_annotations <- any(grepl("^# do_not_plot_annotations", comments))
...
if(any(grepl("orientation: -", comments))){
  colour_win_detect <- colour_minus
  if(is_colour_set){
...
if(is_annotation_set_in_plot_data){
  ...
  if(nrow(annotate_comments_data) > 0){
    ...
    annotate_comments_data$colour[annotate_comments_data$strand == "-"] <- colour_minus_annotate
    annotate_comments_data$colour[annotate_comments_data$strand == "+"] <- colour_plus_annotate
...
plot1 <-  ggplot() +
      ...
      scale_colour_manual(name = NULL,
                              values = c("model" = colour_model, "rawDetect" = "#888888",
                                         "winDetect" = colour_win_detect),
...
if (length(args) >= 3 && exists('annotations') && !is_override_plot_annotations) {
  plot1 <- plot1 +
    geom_segment(
...
if(is_annotation_set_in_plot_data && !is_override_plot_annotations){

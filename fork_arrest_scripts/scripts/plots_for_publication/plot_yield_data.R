...
# Input needed is a space-separated file with columns bin_lo, bin_hi, num_bases
...
r <- ggplot(tbl, aes(x = x, y = num_bases)) +
      geom_bar(stat="identity", position=position_dodge(), fill="#003366") +
      scale_x_continuous(expand = c(0.012, 0.012), limits = c(0, 190)) +
      scale_y_continuous(expand = c(0.012, 0.012), limits = c(1, 30000000000), trans="log10",
                         breaks=trans_breaks("log10", function(x) 10^x)(c(1e1, 1e10)),
                         labels=trans_format("log10", math_format(10^.x))) +
      geom_vline(xintercept = 26, linetype = "dashed", color = "green", linewidth = 4) +
      labs(x = "Read length (kb)", y = "Yield (b)") +
      coord_cartesian(clip = 'off') +
      theme_classic(base_size = 88) +
      theme(axis.text = element_text(colour = "black"),
            axis.title.x = element_blank(),
	        panel.background = element_rect(fill='transparent'),
            plot.background = element_rect(fill='transparent', color=NA))

ggsave(args[1], plot = r, dpi = 400, width = 14, height = 8.4)
